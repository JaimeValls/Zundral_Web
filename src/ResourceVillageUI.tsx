...
export default function ResourceVillageUI() {
  ...
  const lumberRate = useMemo(() => {
    ...
    return getProgression("wood", effectiveLevel, "production");
  }, [lumberMill.level, lumberMill.workers, lumberMill.enabled]);
  ...
  const stoneRate = useMemo(() => {
    ...
    return getProgression("stone", effectiveLevel, "production");
  }, [quarry.level, quarry.workers, quarry.enabled]);

  const foodRate = useMemo(() => {
    if (!farm.enabled || farm.workers === 0) return 0;
    const effectiveLevel = Math.min(farm.level, farm.workers);
    return getProgression("food", effectiveLevel, "production");
  }, [farm.level, farm.workers, farm.enabled]);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      let nextPop = Math.max(1, Math.min(popCap, population + popRate));
      ...
      const nextDivs = divisions.map((d) => {
        ...
        if (dd.status === 'training' && dd.recruited < dd.reqPop && nextPop > 1) {
  ...
  }, [lumberRate, stoneRate, foodRate, foodConsumption, netFoodRate, lumberCap, stoneCap, foodCap, popRate, population, divisions, missions, warehouse.food, farm.stored, popCap]);
  ...
  const [pendingUpgrade, setPendingUpgrade] = useState<
    | null
    | { res: "wood" | "stone" | "food" | "warehouse" | "house"; from: number; to: number; cost: { wood: number; stone: number } }
  >(null);
        ...
        <div className="flex items-center gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline gap-2 flex-wrap">
              ...
              <div className={`text-xs ${popRate < 0 ? 'text-red-500' : 'text-slate-500'}`}>
                {trendText}
              </div>
            </div>
          </div>
      ...
      <div className="sticky top-0 z-50 -mx-4 md:-mx-8 px-4 md:px-8 pb-3 bg-slate-950/95 backdrop-blur">
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          <ResourcePill 
            ...
            rate={popRate} 
            showBar={true}
            trend={popRate > 0 ? `(+${popRate} in 1s)` : popRate < 0 ? `(${popRate} in 1s)` : "(stable)"}
